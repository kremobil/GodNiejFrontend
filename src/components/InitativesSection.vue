      ...
      <router-link :to="`/inicjatywy/${highlightedPost.slug}`" class="promoted_card card" data-aos="zoom-in-right">
        ...
        <div class="card_content" :style="{
          ...
          <div class="image_space">
            <h5 class="singing_in_open" v-if="isActive(highlightedPost)">Trwają zapisy</h5>
          </div>
          <h4 class="card_header">{{ highlightedPost.Tytul }}</h4>
        </div>
      </router-link>
      <router-link :to="`/inicjatywy/${post.slug}`" class="card" :id="`card${index + 1}`"
        v-for="(post, index) in otherPosts" :key="post.id" data-aos="zoom-in-up">
        <div class="card_hover"></div>
        <div class="card_content" :style="{
          ...
          <div class="image_space">
            <h5 class="singing_in_open" v-if="isActive(post)">Trwają zapisy</h5>
          </div>
