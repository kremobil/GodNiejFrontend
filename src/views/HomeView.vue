...
export default {
  ...
  mounted() {
    ...
    godniejBackend.get('/initiatives-section', {
      ...
      (response) => {
        if (!response.data.data.WyroznionyPost) {
          godniejBackend.get('/posts', {
            ...
            (posts) => {
              this.initiativesData = {
                otherPosts: posts.data.data.slice(1, 5),
                title: response.data.data.Tytul,
                button: response.data.data.Przycisk,
                highlightedPost: posts.data.data.slice(0, 1)[0],
              };
            }
          )
        } else {
...
<template>
  <main v-if="aboutUsData && initiativesData && heroData && contactUsData">
    <hero-section :description="heroData.description" :title="heroData.title" :main-button="heroData.mainButton"
      :secoundary-button="heroData.secoundaryButton" :sponsors="heroData.sponsors"
      :sponsors-title="heroData.sponsorsTitle" @open-popup="$emit('openPopup')"></hero-section>
    <initatives-section :button="initiativesData.button" :highlighted-post="initiativesData.highlightedPost"
      :other-posts="initiativesData.otherPosts" :title="initiativesData.title"></initatives-section>
    <about-us-section :title="aboutUsData.title" :button="aboutUsData.button" :description="aboutUsData.description"
      :reviews-header="aboutUsData.reviewsHeader" :reviews="aboutUsData.reviews"></about-us-section>
    <contact-us-section :title="contactUsData.title" :subtitle="contactUsData.subtitle"
      :email-label="contactUsData.emailLabel" :email-placeholder="contactUsData.emailPlaceholder"
      :name-label="contactUsData.nameLabel" :name-placeholder="contactUsData.namePlaceholder"
      :surname-label="contactUsData.surnameLabel" :surname-placeholder="contactUsData.surnamePlaceholder"
      :message-label="contactUsData.messageLabel" :message-placeholder="contactUsData.messagePlaceholder"
      :button="contactUsData.button"></contact-us-section>
  </main>
